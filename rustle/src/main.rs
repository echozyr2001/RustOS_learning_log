...
impl RustleGame {
    ...
    fn display_invalid_letters(&self) {
        if !self.guessed_letters.is_empty() {
            print!("Letter not in the world:");
            self.guessed_letters.iter().for_each(|l| print!("{}", l));
            println!();
        }
    }
    ...
    fn ask_for_guess(&mut self) -> String {
        ...
        while !valid_guess {
            ...
            if guess.len() != WORD_LENGTH {
                ...
                println!("{}", format!("{guess} is not in the dictionary.").red())
            } else {
    ...
    fn is_game_over(&self, guess: &str) -> bool {
        ...
        if guess == self.word {
            println!(
                "{}",
                format!("Correct! You guessed the word in {n_tries} tries.").bright_green()
            );
            true
